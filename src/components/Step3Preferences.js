...
const Step3Preferences = ({ formData, updateForm, nextStep, prevStep, resetAddress }) => {
  ...
  useEffect(() => {
    fetch('https://backend-edvinbweb-comps-projects-9855a82b.vercel.app/api/countries')
      .then(res => res.json())
      .then(setCountries)
  ...
  useEffect(() => {
    if (formData.country) {
      fetch(`https://backend-edvinbweb-comps-projects-9855a82b.vercel.app/api/states/${formData.country}`)
        .then(res => res.json())
        .then(setStates)
  ...
  useEffect(() => {
    if (formData.state) {
      fetch(`https://backend-edvinbweb-comps-projects-9855a82b.vercel.app/api/cities/${formData.state}`)
        .then(res => res.json())
        .then(setCities)
