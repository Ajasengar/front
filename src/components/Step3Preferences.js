...
const Step3Preferences = ({ formData, updateForm, nextStep, prevStep, resetAddress }) => {
  ...
  useEffect(() => {
    fetch('https://backend-ashy-beta-38.vercel.app/api/countries')
      .then(res => res.json())
      .then(setCountries)
  ...
  useEffect(() => {
    if (formData.country) {
      fetch(`https://backend-ashy-beta-38.vercel.app/api/states/${formData.country}`)
        .then(res => res.json())
        .then(setStates)
  ...
  useEffect(() => {
    if (formData.state) {
      fetch(`https://backend-ashy-beta-38.vercel.app/api/cities/${formData.state}`)
        .then(res => res.json())
        .then(setCities)
